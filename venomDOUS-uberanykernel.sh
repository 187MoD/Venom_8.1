...
BOEFFLA_VERSION="-UberTC-beta1.4_kltedous*Los.15.1"

TOOLCHAIN="/home/skater187/toolchains/uberTC/bin/arm-linux-androideabi-"
ARCHITECTURE=arm
...
DEFCONFIG="boeffla_klteduos_defconfig"
...
step0_copy_code()
{
	...
	sed "s/\`echo \$LINUX_COMPILE_BY | \$UTS_TRUNCATE\`/$KERNEL_NAME-$BOEFFLA_VERSION-$BOEFFLA_DATE-[supported-by-The~Skater~187]/g" -i $BUILD_PATH/scripts/mkcompile_h
	sed "s/\`echo \$LINUX_COMPILE_HOST | \$UTS_TRUNCATE\`/187-Mod/g" -i $BUILD_PATH/scripts/mkcompile_h
}
